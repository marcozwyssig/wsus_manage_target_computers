...
$logFile = "c:\ProgramData\Atos\wsus_target_computer.log"
...
function ConfigureTargetComputer($targetcomputers, $wsusServer, $parentGroup){
    foreach ($targetcomputer in $targetcomputers) {
        if ($targetcomputer -is [System.String] ) {
            $wsustargetcomputer = $wsusServer.GetComputerTargetGroups() | Where-Object { $_.Name -eq $targetcomputer }
            if( $null -eq $wsustargetcomputer ) {
                if ($null -eq $parentGroup) {
                    Write-Log "Creating new target computer ${targetcomputer}"
                    $wsustargetcomputer = $wsusServer.CreateComputerTargetGroup($targetcomputer)
                } else {
                    $parentGroupName = $parentGroup.Name
                    Write-Log "Creating new target computer ${targetcomputer} in parent $parentGroupName"
                    $wsustargetcomputer = $wsusServer.CreateComputerTargetGroup($targetcomputer, $parentGroup)
                }
            } else {
                Write-Log "Target computer '${targetcomputer}' exists"
            }
        }

        if ($targetcomputer -is [System.Array] ) {
            ConfigureTargetComputer -targetcomputers $targetcomputer -wsusServer $wsusServer -parentGroup $wsustargetcomputer
        }
    }
}

try {
    $wsus = [System.Reflection.Assembly]::LoadWithPartialName('Microsoft.UpdateServices.Administration')
    $wsusServer = [Microsoft.UpdateServices.Administration.AdminProxy]::GetUpdateServer()

    $targetcomputers = Get-Content "c:\ProgramData\Atos\targetgroups.json" | ConvertFrom-Json
    ConfigureTargetComputer -targetcomputers $targetcomputers -wsusServer $wsusServer -parentGroup $null
